...
def _migrate_add_mimo_session_id_column():
    """Add mimo_session_id column to sessions table if it doesn't exist.

    Optional durable lazy-cache: stores the mimo ses_… id for pre-migration
    rows whose primary key is still a uuid4.
    """
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
    ...
    try:
        ...
        cursor = conn.execute("PRAGMA table_info(sessions)")
        columns = [row[1] for row in cursor.fetchall()]
        if "mimo_session_id" not in columns:
            conn.execute("ALTER TABLE sessions ADD COLUMN mimo_session_id TEXT")
            conn.commit()
            logging.getLogger(__name__).info("Migrated: added 'mimo_session_id' column to sessions")
    except Exception as e:
        logging.getLogger(__name__).warning(f"Migration check for mimo_session_id failed: {e}")
    finally:
        try:
            conn.close()
        except Exception:
